...
bool isValidId(const string& id) {
    ...
    void viewContacts(){
        ...
        cout << left << setw(10) << "ID" 
             ...
             << setw(40) << "Email" << setw(10) << "Age" 
             << setw(15) << "Birthday" << setw (15) << "Category" << setw(25) << "Emergency Contact"
             << setw(35) << "Address" << "\n";
        cout << string(155, '-') << "\n";
            ...
            for (size_t i = 0; i < contacts.size(); ++i) {
                ...
                cout << left << setw(10) << contact.getId()
                     ...
                     << setw(40) << contact.getEmail()
                     << setw(10) << contact.getAge()
                     << setw(15) << contact.getBirthday()
                     << setw(15) << contact.getCategory()
                     << setw(25) << contact.getEmergencyContact()
                     << setw(35) << contact.getAddress() << "\n";
            }       
        }
...
int extractNumericId(const string& id) {
    size_t pos = 0;
    while (pos < id.size() && !isdigit(id[pos])) ++pos; 
...
private:
    ...
    int partition(vector<Contact>& contacts, int low, int high) const {
        int pivot = extractNumericId(contacts[high].getId());
        int i = low - 1;

        for (int j = low; j < high; ++j) {
            if (extractNumericId(contacts[j].getId()) <= pivot) {
                ++i;
